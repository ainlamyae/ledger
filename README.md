...
- Four cards, in this order: **Status**, **Macros**, **Activity**, **Sleep** — Macros/Activity/Sleep mirror the Health Indicator charts below them. Each is a stack of label/value rows rather than one figure, and every row reads green on the right side of its target, red otherwise, grey when nothing's logged — the same target functions the chart underneath it uses, so a card can't disagree with its own chart.
...
- Card order: Status (Body Mass/BMI/Change/Balance/Estimate), Macros (Calory/Protein/Fiber/Fat/Carb), Activity (Duration/Calories), Sleep (Duration/Window).
