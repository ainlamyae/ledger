...
- **Intake Macros** — one card, one row per metric:
  - **Intake** — today's calories (`withExplicitSign`) vs. the day's calorie limit (`getCalorieTarget`), separated by `<`; green when under, red over.
  - **Digestion** — TEF expenditure (signed negative) vs. the TEF the calorie target would produce at the same `tefDivisor()` rate — `calTarget.kcal × (1 − tefDivisor())`.
...
- **Physical Activity** — four rows:
  - **Activity** — total kcal burned (signed negative) vs. `getActivityTargetKcal`; green when the target is met.
