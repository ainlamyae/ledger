...
- Four cards, in this order: **Status**, **Intake Macros**, **Physical Activity**, **Sleep** — the last three mirror the Health Indicator charts below them by name. Each is a stack of label/value rows rather than one figure, and every row reads green on the right side of its target, red otherwise, grey when nothing's logged — the same target functions the chart underneath it uses, so a card can't disagree with its own chart.
- **Status** — five rows summarising today's energy state:
  - **Body Mass** (`83 / 70 kg`, today vs. `BODY_MASS_TARGET_KG`) — colored by whether today is on the correct side of the target for a cut or a bulk, via `bodyMassTargetIsDownward`.
  - **BMI** (`28.9 / 24.7 kg/m²`, today and target) — computed from body mass and `HEIGHT_CM` via `computeBmi`; coloured the same way as Body Mass.
  - **Maintenance** — BMR as a signed expenditure, same sign convention as the Calorie Balance tooltip's own Maintenance line.
  - **Balance** (`-380 / -450 kcal`) — today's net kcal against `targetBalanceKcal(planBodyMassKg(entries))` — the same formula the Calorie Balance chart plots, just for today alone.
  - **Estimate** (`132 (2027-01-21)`) — day count and arrival date read straight off `calcProjection`, the same figures State Trend & Forecast's own time-progress meter shows. Reads "Reached" once there, `—` whenever no projection can be drawn (no profile, wrong direction, fewer than two weigh-ins).
- **Intake Macros** — one card, one row per metric:
  - **Intake** — today's calories vs. the day's calorie limit (`getCalorieTarget`), separated by `/`; green when under, red over.
  - **TEF** — TEF expenditure (signed negative) vs. the TEF the calorie target would produce at the same `tefDivisor()` rate — `calTarget.kcal × (1 − tefDivisor())`.
  - **Protein**, **Fiber**, **Fat** and **Carb** each show as a **band** (`53 / 112~154 g`) from their own `get*TargetBandG` function. Protein, Fiber and Fat all score the same way — under the floor is red, in the band is green, over the ceiling is a darker green rather than a miss. Carb reads the opposite way: under the floor is grey/unscored rather than a miss, and it's the ceiling that turns red — matching the Carbohydrate Intake chart's own bar coloring, since too little carbohydrate isn't the deficiency too little fiber or protein is.
- **Physical Activity** — four rows:
  - **Activity** — total kcal burned vs. `getActivityTargetKcal`, both shown as a burn (a leading `-`, since kcal here is always an expenditure) except a true zero, which reads plain `0` rather than `-0`; green when the target is met.
  - **Cardio**, **NEAT** and **Strength** — the same three buckets the Physical Activity chart itself stacks by (the `Activity` sheet's own Category column, via `physiqueActivityByCategory`), matched by prefix rather than an exact string so `Strength` and `Strength Training` both land in the same row. Each reads `N min (-N kcal)`, purely informational — no target, so no green/red — and reads `0 min (0 kcal)` (no sign on a true zero) rather than `—` for a bucket nothing was logged into today, since that's a real zero, not a missing reading.
- **Sleep** — three rows:
  - **Deprivation** — today's sleep-deprivation cost in kcal (`dailyEnergyBalanceKcal`), coloured on the same green→red gradient the Sleep chart's dot uses (`sleepDeprivationDotColor`); target is always `0 kcal` (no sleep debt).
  ...
  - **Window** (`01:16-08:58`, that day's `sleepBedMin`/`sleepWakeMin` as one range rather than two rows).
...
- **Box order is the order the algebra runs**: the profile and the knobs (`m`, `m̄`, `h`, `a`, `σ`, `BMR`, `M`, `MET`, `τ`, `κ`, `Δm%`, `Δm`), then `s` and `γ` (sleep length and the sleep-deprivation rate, with `δ` right after them — readonly, the kcal short sleep adds to `D`), the two population constants (`ρ`, `ε`), then `D` (readonly — the deficit `Δm` and `ρ` imply, divided by however much `δ` inflated it, sitting between the two figures it's made of and the intake it comes off), `f` and the answers it feeds — `Eᵢₙ`, `TEF`, then the journey block (`m_g` and `BMI_g`, the destination you choose, immediately above the `t` and arrival date that measure the trip to it), the adaptation block, the lean-mass block (`LBM`, the `p_min`/`p_max` rule, then the `P_min`/`P_max` grams it produces — the rule sits with the grams rather than up among the other editable knobs, so `p × LBM = P` reads as one derivation instead of a multiplication split across the modal), the fiber band right after it (`f_min`/`f_max`, then the `F_min`/`F_max` grams they produce), the fat band after that (`k_min`/`k_max`, then `G_min`/`G_max`), and the carb band after fat (`q_min`/`q_max`, then `C_min`/`C_max`), with glycogen closing the list.
...
- **Sleep Efficiency Factor** (`s`/`PLAN_SLEEP_HOURS`, `γ`/`SLEEP_DEPRIVATION_PCT_PER_HOUR`, default **2.5**%/hr — the midpoint of the 2–3%/hr literature range) — `η = 1 − (γ/100) × max(0, SLEEP_TARGET_HOURS − s)`, and short sleep needs a *bigger* raw deficit to still deliver the same real fat loss: `D = rawD / max(η, 0.2)` (floored so a typed extreme can't send the target to ±Infinity), only when `rawD` is an actual deficit — a bulk target isn't made more effective by poor sleep. `s` defaults to `SLEEP_TARGET_HOURS` itself, i.e. "assume you hit it" — an untouched box keeps `η = 1` and every downstream figure byte-identical to before this existed.
  - `δ` (readonly, `Deprivation Effect`) is `D − rawD` — the kcal short sleep actually added to the deficit, shown in its own box and, when nonzero, its own trace row (`η`, then `δ`) right after `Eₐ`.
  - Lives in `calorieTargetDetail` (`wellness-math.js`), the one function `getCalorieTargetKcal` always calls — so this isn't a playground-only preview: the real site-wide `Eᵢₙ`, `targetBalanceKcal` (Calorie Balance's target dash and the Sleep chart's own dot), and the Health Plan prompt's `D` line all read the same sleep-adjusted figure. TAU mode (`Solve for → Activity`) runs the identical forward adjustment through the same `sleepAdjustedDeficitKcal` call; Target body mass and Weekly fat loss reverse-solve `D` **from** a typed `Eᵢₙ` instead of building it from a target rate, so `δ` reads `—` in those two modes rather than guessing a direction for it.
...
- **The fiber band follows protein**, same shape but two different bases rather than one shared one: `F_min = f_min × (Eᵢₙ / 1000)` (a floor scaled to how much you eat — 14 g per 1,000 kcal, the USDA/DGA rule of thumb) and `F_max = f_max × m` (a ceiling scaled to body weight — 0.5 g/kg). Both `f_min`/`f_max` are editable, and — unlike protein's `LBM` — neither box rides on one the other computes.
  - `F_min` reads `Eᵢₙ` straight off the sheet's own box rather than re-deriving it: by the time the fiber block renders, that box already holds this render's target — typed or solved, in every mode.
  - **Save writes the grams**, to `FIBER_TARGET_G_MIN` / `FIBER_TARGET_G_MAX`, which outrank the flat `FIBER_TARGET_G` everywhere the fiber target is read (tile, chart). The `f_min`/`f_max` rule is saved too (`FIBER_G_PER_1000_KCAL_MIN` / `FIBER_G_PER_KG_MAX`), same reasoning-next-to-result rule as protein's `p_min`/`p_max`.
...
- **A `Target` row sits directly under each table's Total, dashed off from it rather than bold-bordered like Total** (`renderCalcBreakdown`/`renderPhysiqueActivityBreakdown`), so what the day actually did has an in-table goal to read against. The Consumption table's Target reuses the exact figures the Wellness tiles/charts already compute — `getCalorieTarget`, `getProteinTargetBandG`/`getFiberTargetBandG` (band floor only, not the full range — a `109~133` reads too wide for this table's narrow columns), and the FDA Daily Value reference for Fat/Carbohydrate (`nutrient-targets.js`, no personalized formula for either yet). The Workout table's Target is `getActivityTargetMin`/`getActivityTargetKcal` at today's Body Mass — the same pinned-vs-flat `ACTIVITY_TARGET_MIN` rule the Physical Activity tile and chart use, so neither table's goal can drift from what "hitting it" means elsewhere in the app.
...
- After Calculate, an **activity table** under the Workout field shows the working per exercise — its quantity, the MET it was priced at, its **Dur**ation and its **Cal**ories, same abbreviated headers the Physique table's own Dur/Cal↑ columns use — summed to a Total row, with the day's Duration/Calorie goal in a `Target` row directly under it (see [Health — Physique](#health--physique)). Nothing about it is stored; it's local arithmetic over the Workout text, recomputed whenever the day is opened.
...
- `PLAN_SLEEP_HOURS` defaults to `SLEEP_TARGET_HOURS` itself — `η = 1`, `D = rawD`, byte-identical to the target this app always produced. See the [Formula Playground](#health--formula-playground)'s `s`/`γ`/`δ` fields for where it's tuned.
...
- **Measured wins over estimated.** `TEF = f × intake` is the fallback — whenever a day's Physique row has its own TEF column calculated (see [`Physique`](#physique)), that figure is used instead, and the tooltip's Digestion row says which one produced the bar (`measured` vs. `est.`). State Trend & Forecast's gray Calorie-Implied Trajectory line gives the same day the same precedence, so the two charts can't disagree about which TEF a given day used.
...
- Card order: Status (Body Mass/BMI/Change/Balance/Estimate), Intake Macros (Calories/Protein/Fiber/Fat/Carb), Physical Activity (Duration/Calories/Cardio/NEAT/Strength), Sleep (Duration/Window).
- Sums today's entries per category; Fiber/Fat/Carb each read the day's own persisted Physique figure (`fiberG`/`fatG`/`carbG`).
- Green/red by `withinCalorieTarget`, `withinProteinBand`, `withinFiberBand`, `withinFatBand`, `withinCarbBand`, `mins ≥ getActivityTargetMin(latest body mass)`, `kcal ≥ getActivityTargetKcal(latest body mass)`, and (Status) which side of `BODY_MASS_TARGET_KG`/the daily rate/the balance target `bodyMassTargetIsDownward`/`weeklyFatLossKgAt`/`targetBalanceKcal` call for.
- Carb's under-band case reads grey, not red — same as its chart's bar coloring.
- Physical Activity's Calories row sums `activityEntryKcal` per entry rather than just the logged minutes, so it agrees with the Activity/Calorie Balance charts. Cardio/NEAT/Strength split that same set of entries by the `Activity` sheet's Category column (prefix-matched), each unscored (`N min (N kcal)`, `0 min (0 kcal)` rather than `—` for a bucket with nothing logged — that's a real zero, not a missing reading).
- Status's BMI is `computeBmi` at today's reading and at target; Change is `computeBodyMassTrendSlopeGramsPerDay`'s latest point against `weeklyFatLossKgAt` ÷ 7; Balance is intake − BMR − activity − TEF against `targetBalanceKcal(planBodyMassKg(entries))`, the same arithmetic the Calorie Balance chart plots for one day; Estimate is `calcProjection`'s day count and ETA — "Reached" once there, `—` when no projection can be drawn.
