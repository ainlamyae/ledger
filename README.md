...
| 16 | `wellness.js` | Health Tracker: filterable/sortable Health Log table, category-aware Log Entry form with history-based autocomplete, CRUD against the `Wellness Log` tab, multi-select bulk Edit/Recalculate/Merge, and wiring the Calculate button to `calorie-estimator.js` — chart rendering itself lives in `charts.js` | `initWellness`, `refreshWellness` |
