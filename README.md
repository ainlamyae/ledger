...
- **Fat Intake** — same scored bars and same three-way split as Fiber (`getFatTargetBandG`/`withinFatBand`), but the band edges draw as one continuous dashed line across the whole window (`targetMarkColor()`, the same near-black/near-white every target cap on the panel uses) rather than Fiber's per-day caps and shaded fill — Fiber's floor and ceiling come off two different bases, so a fill between them reads as the band itself; Fat's two ends are both a share of the same Eᵢₙ, so a flat line at each edge already says "this held all window" without a fill repeating it. Falls back to a flat 65g (the USDA %DV reference for a 2,000 kcal diet), same shape as Fiber's own flat fallback, until the Formula Playground's fat band is saved.
...
- **Select rows, then Calculate / TEF / Combine & Sort in the bulk bar** to run the same actions across whole days at once. Calculate uses the same two estimators and incremental reuse as the form's own button — a line whose `noteLine` still matches that day's saved breakdown keeps its numbers, so re-running a stretch of days only pays for what changed. TEF re-estimates from each selected day's *already-saved* breakdown, same as the form's own TEF button, and never touches Groq/USDA either.
