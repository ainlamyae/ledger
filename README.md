...
- **Status** — everything about today's Body Mass reading, five rows:
  ...
  - **BMI** (`28.7 / 24.2 kg/m²`) — `computeBmi` at today's reading and at the target, colored the same direction as Body Mass; `—` without a `HEIGHT_CM`.
  - **Change** (`-108 / -120 g/day`) — today's smoothed slope (`computeBodyMassTrendSlopeGramsPerDay`, the same figure the Body Mass chart's own "Smoothed Change" tooltip line shows) against the Formula Playground's weekly fat-loss rate (`weeklyFatLossKgAt`) converted to a daily figure, both signed negative for losing.
  - **Balance** (`-380 / -450 kcal`) — today's net kcal (intake minus BMR minus activity minus TEF, measured TEF winning over the flat estimate) against `targetBalanceKcal(planBodyMassKg(entries))` — the same formula the Calorie Balance chart plots, just for today alone.
...
- **Intake Macros** — one card, one row per metric. The **Calories** row uses `<` rather than `/` between actual and target, since the app's read of a calorie figure is a threshold, not a band. **Protein**, **Fiber**, **Fat** and **Carb** each follow as a **band** (`53 / 112~154 g`) from their own `get*TargetBandG` function. Protein, Fiber and Fat all score the same way — under the floor is red, in the band is green, over the ceiling is a darker green rather than a miss. Carb reads the opposite way: under the floor is grey/unscored rather than a miss, and it's the ceiling that turns red — matching the Carbohydrate Intake chart's own bar coloring, since too little carbohydrate isn't the deficiency too little fiber or protein is.
- **Physical Activity** — five rows:
  - **Duration** (`actual / target min`, target from `getActivityTargetMin`) and **Calories** (`actual / target kcal`). Calories sums `activityEntryKcal` per logged entry — the same per-entry rule the Activity/Calorie Balance charts and Insight use (a Calculate-derived figure wins, else a flat per-minute estimate) — against `getActivityTargetKcal`, so it can't disagree with what hitting the Duration target would burn.
  - **Cardio**, **NEAT** and **Strength** — the same three buckets the Physical Activity chart itself stacks by (the `Activity` sheet's own Category column, via `physiqueActivityByCategory`), matched by prefix rather than an exact string so `Strength` and `Strength Training` both land in the same row. Each reads `N min (N kcal)`, purely informational — no target, so no green/red — and reads `0 min (0 kcal)` rather than `—` for a bucket nothing was logged into today, since that's a real zero, not a missing reading.
- **Sleep** — Window (`01:16-08:58`, that day's `sleepBedMin`/`sleepWakeMin` as one range rather than two rows), Duration (colored on the same red→amber→green gradient the Sleep chart's bars use, `sleepStatusColor`, rather than a flat green/red split), then **Deprivation Effect** — the kcal a short night cost the day's own balance (`dailyEnergyBalanceKcal`, the same figure the Calorie Balance chart's tooltip line and State Trend & Forecast's gray trajectory now fold in), coloured on a green→red gradient scaled to that same kcal figure (`sleepDeprivationDotColor`) rather than the duration gradient beside it.
...
- **State Trend & Forecast** — body mass history, smoothed trend, and a projection toward target; optional BMI twin axis.
  ...
  - **A transparent amber zone marks the glycogen + water swing** (`ΔM_gly`, the same figure the Formula Playground's glycogen block computes at its default knobs) — the day-to-day wobble glycogen and its bound water can account for on their own. Anchored to the Calorie-Implied Trajectory line below rather than centred on the smoothed trend: that trajectory assumes intake vs. burn is the only thing moving the scale, so it's the driest the trend could read, and the trend's own wobble above it is exactly the glycogen/water noise the zone exists to mark. The zone's top edge sits exactly on the trajectory line; the band runs from there down by the swing figure. Needs a full profile (height/age/sex) to exist at all, the same guard the trajectory line below needs — there's nothing to anchor to without it.
...
- **Body Mass** — one bar per reading, scored by direction of travel; left axis restates it as stored fat energy.
  ...
  - The hover's **`Changed Mass`** reads in grams, not kg — small enough day to day that a two-decimal kg figure undersells it — and a **`Smoothed Change`** row under it gives the green trend line's own slope at that point in g/day (`computeBodyMassTrendSlopeGramsPerDay`): a centred difference between its neighbouring logged points, divided by the calendar gap between them (one-sided at either end of the series). Reading the *smoothed* line's slope instead of the raw reading-to-reading delta is the same reason the line is smoothed to begin with — a single water- or glycogen-heavy weigh-in swings the raw number without the underlying trend having actually turned.
...
**Tune**, in the Physique panel heading, opens it.
...
- **Tune**, opening the Formula Playground, sits in this heading too rather than Health Indicator's — the two panels' actions collapsed onto the one that actually edits the numbers the other only charts.
