...
- **TEF** (`estimateTefBreakdown`, `micronutrient-insight.js`) reads each Breakdown ingredient's own protein/carb/fat grams straight off the same pulled 🧬 Micronutrients panel `Nutrition`'s Micronutrients column carries, scaled to how much of it was actually logged (the same per-ingredient scaling `aggregateMicronutrientIntake` uses for the Health Insight Micronutrients mode), so it works on a day still open in the form and never re-runs Consumption through Groq/USDA. Calculate draws its own three-row Protein/Carbohydrate/Fat table (`physique-tef-breakdown`) above the hidden TEF field whenever at least one ingredient has macros pulled; a day with none pulled yet is left exactly as it was — a day with no 🧬 data reads as "not calculated", not as a measured zero. The **TEF** bulk button (beside Combine & Sort) re-estimates it for already-selected, already-saved days without touching Consumption, Groq or USDA at all — for catching up days logged before this column existed.
