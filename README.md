...
| E — Amount | Number, a `;`-joined pair, or a `/`-joined bed/wake pair | The measured value; units defined by column F. For `Calories; Protein` rows, two values joined as `"<kcal>; <protein g>"` (e.g. `"320; 10"`) instead of a single number. For `Sleep` rows, either a plain number of hours, or bed/wake clock times joined as `"<bed HH:MM>/<wake HH:MM>"` (e.g. `"23:30/07:00"`) — duration is then computed automatically (wrapping past midnight when wake ≤ bed) and stored as the entry's hours for every existing consumer (today-glance tile, projection, calibration), while the bed/wake pair itself only feeds the Rest & Recovery chart's floating bars |
