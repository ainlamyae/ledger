...
flowchart TD
    ...
    Groq["Groq chat-completions API<br/>api.groq.com<br/>🧮 Calculate ingredient extraction<br/>💡 Insight report generation<br/>🥗 Food Insight report generation<br/>📈 Activity Insight report generation"]
...
flowchart TD
    ...
    FoodInsight --> FoodPreview["Client-side preview:<br/>aggregate Calculate breakdown items<br/>over custom From/To range, count→grams<br/>via Nutrition Facts — no API call"]
    ...
    Idle --> ActivityInsight["📈 Activity Insight<br/>Weight Trend & Forecast"]
    ActivityInsight --> ActivityPreview["Client-side preview:<br/>aggregateWindow/previousDateRange +<br/>per-muscle-group sets/sessions/last-trained<br/>over custom From/To range — no API call"]
    ActivityPreview --> ActivitySend{"🚀 Send to AI<br/>clicked?"}
    ActivitySend -- yes --> ActivityGroqReport["Groq chat-completions API<br/>renders free-text performance report<br/>— nothing saved"] --> Idle
    ActivitySend -- no --> Idle
...
| 18 | `strength-plan.js` | Strength Training Plan panel: three Leg/Push/Pull gym tables (machine, sets × reps, rest), each exercise with a "Done" checkbox. The single Log Workout button below all three reads every ticked box across all of them (so a session mixing exercises from different tables still logs as one entry), estimates its duration from each ticked row's sets/reps/rest plus a per-exercise transition and a fixed warm-up allowance, and pre-fills `wellness.js`'s Log Entry modal (Category, Description, Amount, Unit, and a per-day breakdown in Notes) for the user to review and save — it never writes to the sheet itself | `initWorkoutPlan` |
...
| 20 | `settings-panel.js` | Settings panel: Key/Value/Notes table below Contact List, add/edit/delete against the `Settings` tab, re-applying `app.js`'s `loadSettings`/`applySettingsToWidgets` after every change so live widgets/targets pick up the new value immediately. Also home to `saveSettingValues`, the shared writer every feature that persists a computed result uses (calibration, the three AI insight panels). Both write paths use `RAW` and reset the Value cell's number format — see [Data Model](#data-model) for the date-format bug that made necessary | `initSettingsPanel`, `refreshSettingsList`, `saveSettingValues`, `clearSettingValueFormats` |
| 21 | `travel.js` | Travel: sortable/searchable Travel List table, add/edit/delete, and pairing Arrival/Departure rows into the Time Spent by Country and Countries Visited data `charts.js` renders | `initTravel`, `refreshTravel` |
...
| 23 | `calibration.js` | Weight Trend & Forecast's Calibrate button: fits `charts.js`'s projection formula (energy density, activity, and sleep gains, plus a baseline drift term) to the user's own historical Weight/Calories/Activity/Sleep entries via weighted least squares, and writes the resulting gains to the `Settings` tab (using the same row-diff/update/append pattern `settings-panel.js` uses) so `charts.js`'s `calcProjection` picks them up on the next render. The save is verified rather than assumed: every written coefficient is read back off the sheet and a partial write is reported by key, since the formula needs all four core gains present or every consumer silently falls back to the generic one. Also owns the Health Metrics **📐 Calibrated / 📊 Generic formula** toggle, which flips a single flag `charts.js`'s `getCalibratedGains()` reads — no parallel "generic mode" code path exists | `initCalibrationPanel`, `runCalibration`, `saveCalibratedGains`, `resetCalibration`, `initFormulaToggle`, `refreshFormulaToggle` |
| 24 | `insight.js` | Weight Trend & Forecast's 💡 Insight button: aggregates the selected custom From/To date range (plus the immediately preceding period of the same length, for trend comparison) from `allWellnessEntries` — including activity broken down per Description (e.g. NEAT/Cardio/Strength Training), each with its own avg minutes/day and avg kcal/day burned (where logged) and trend, alongside the combined total — reuses `charts.js`'s `calcProjection`/`getCalibratedGains` for the trajectory line and calibrated energy density, owns the shared age/sex/height/weight/BMI profile block (`gatherProfileSnapshot`/`formatProfileLines`) that Food and Activity Insight send too, passes the protein target through as a range (`131-164`) rather than a single figure so an in-band day isn't reported as short of target, labels the calorie figure `max:`/`min:` rather than `target:` (with a system-prompt rule spelling out that under a `min` is not a win and under a `max` is not a shortfall), and — only once "🚀 Send to AI" is clicked — sends the resulting plain-language summary to Groq and renders the free-text report | `initInsightPanel` |
| 25 | `food-insight.js` | Weight Trend & Forecast's 🥗 Food Insight button: aggregates every Calculate-derived breakdown item across `allWellnessEntries` in the selected custom From/To date range into one per-ingredient total (name, amount, calories, protein), converting a unit-count total to grams via that ingredient's own per-unit weight in the Nutrition Facts table (`nutrition.js`'s `findNutritionEntry`/`parseGramsFromAmount`/`parseCountFromAmount`) so the summary reads in one consistent unit — and, only once "🚀 Send to AI" is clicked, sends that list, the shared profile block from `insight.js` (reference intakes for iron/calcium/folate/B12/protein all depend on sex and age, so an ingredient list alone can't be judged), plus an optional free-text question to Groq and renders the free-text nutrient-gap report. A separate feature/modal from `insight.js` by design; no vitamin/mineral data is fetched or stored anywhere in this app, so this leans entirely on the model's own general food-composition knowledge | `initFoodInsightPanel` |
| 26 | `activity-insight.js` | Activity Insight panel's 📈 button: reuses `insight.js`'s `gatherProfileSnapshot`/`formatProfileLines` for the same profile block the other two panels send (what counts as heavy volume depends on the body lifting it) and its `aggregateWindow`/`previousDateRange` for the selected custom From/To range's activity-type breakdown and current-vs-previous-period consistency, adds its own per-muscle-group breakdown (Legs/Chest/Back/Shoulders/Biceps/Triceps) by walking every logged workout note through `activity-estimator.js`'s `parseWorkoutNoteLines` and a fixed exercise-name → muscle-group lookup table, computing sets/sessions performed in range plus a last-trained date searched across full history — and, only once "🚀 Send to AI" is clicked, sends the resulting plain-language performance summary to Groq and renders the free-text report | `initActivityInsightPanel` |
| 27 | `protein-rotation.js` | Protein Source Rotation panel: for every ingredient flagged with a Protein % on its Nutrition Facts row, sums actual protein eaten from it in the selected custom From/To date range against a live target (that ingredient's share of the current protein target, via `charts.js`'s `getProteinTargetG` — the midpoint of the goal-weight-derived target band, since a share-of-target split needs one denominator to divide up), rendered as a horizontal bar chart sorted by remaining gap descending — the source with the most left to eat leads, anything at or past its target sinks to the bottom | `initProteinRotationPanel`, `renderProteinRotationChart` |
...
11. Opening the 💡 Insight modal computes the data preview client-side (the shared age/sex/height/weight/BMI profile block from `gatherProfileSnapshot`/`formatProfileLines`, selected-range + previous-period aggregation from `allWellnessEntries`, and `charts.js`'s `calcProjection`/`getCalibratedGains` for the trajectory line) — no API call yet, and changing the From/To date range just recomputes this preview.
...
13. Opening the 🥗 Food Insight modal computes the same profile block (shown above the table as a "Sent with your food:" line, so nothing is attached to the request invisibly) plus an ingredient summary client-side — every Calculate breakdown item across `allWellnessEntries` in the selected custom From/To date range, grouped by name, with a unit-count total converted to grams via `nutrition.js`'s Nutrition Facts lookup — no API call yet.
...
**AI Activity Insight** (user-triggered, `activity-insight.js`)
15. Opening the 📈 Activity Insight panel computes a workout-performance summary client-side for the selected custom From/To range — reusing `insight.js`'s `gatherProfileSnapshot`/`formatProfileLines` for the profile block and its `aggregateWindow`/`previousDateRange` for consistency and activity-type totals, plus a per-muscle-group sets/sessions/last-trained breakdown derived by parsing every logged workout note (`activity-estimator.js`'s `parseWorkoutNoteLines`) through a fixed exercise → muscle-group lookup table — no API call yet.
16. Only clicking "🚀 Send to AI" sends that summary, plus your question (or nothing, if left blank), to Groq's chat-completions API and renders the free-text performance report; nothing about this step touches the spreadsheet or any cache, and it never affects the (separate) Wellness/Food Insight panels' own state.

**Manual refresh**
17. The 🔄 Refresh and 🧹 Clear Cache buttons — part of the floating action button stack in the bottom-right corner — clear the cache and re-fetch everything; Clear Cache additionally clears Cache Storage and unregisters any service workers, then reloads — for recovering from a stale deployed version. The same stack also holds the 🌙/☀️ dark mode toggle and the expand/collapse-all panels button.
...
| `CALORIE_TARGET_KCAL` | `2000` | Caloric Intake chart bound line — read as a daily **maximum** while your goal weight is below your latest weigh-in and a **minimum** while it's above (the key keeps its historical name for backwards compatibility with existing sheets; the app never presents it as a target); caloric-balance input for the Weight Trend & Forecast projection. Ignored if `HEIGHT_CM`, `BIRTH_DATE`, `SEX`, `ACTIVITY_MULTIPLIER`, and `WEEKLY_FAT_LOSS_KG` are all set and a Weight entry has been logged — see below |
| `SEX` | *(none — falls back to `CALORIE_TARGET_KCAL`)* | `male` or `female` — the Mifflin-St Jeor BMR constant (`+5`/`−161`) used both when calculating a personalized calorie bound (see `ACTIVITY_MULTIPLIER` below) and for the Calorie Balance & Body Change chart's per-day maintenance figure, plus the sex term in Body Composition Change's body-fat estimate; also sent in the profile block all three AI panels use, since calorie, protein and micronutrient reference intakes all differ by sex |
...
| `WEEKLY_FAT_LOSS_KG` | *(none — falls back to `CALORIE_TARGET_KCAL`)* | Desired rate of fat loss in kg/week, e.g. `0.5` — `getCalorieBoundKcal()` converts this to a daily deficit (using the calibrated energy density once available, `GENERIC_KCAL_PER_KG_FAT` — 7,700 — otherwise) and subtracts it from the calculated maintenance calories from `ACTIVITY_MULTIPLIER` to get the actual daily figure used everywhere. Set to a negative value for a lean bulk instead of a cut — which raises the figure above maintenance, and (if you're already at your goal weight, so there's no `WEIGHT_GOAL_KG`-vs-weigh-in direction to read) is also what flips it from a maximum to a minimum |
...
| `PROJ_CAL_KG_PER_KCAL_DAY` | *(none — generic formula used)* | Calibrated kg/day of weight-trend change per kcal/day of intake above the daily calorie bound (`CALORIE_TARGET_KCAL`) (replaces the generic formula's fixed 7,700 kcal/kg) |
| `PROJ_ACTIVITY_KG_PER_KCAL_DAY` | *(none — generic formula used)* | Calibrated kg/day per kcal/day burned through activity — real Calculate-derived kcal (`Activity; Calories` entries' second Amount value) where logged, falling back to a flat 5 kcal/active-minute estimate for any entry without one, same units either way so this term is directly comparable to `PROJ_CAL_KG_PER_KCAL_DAY` above. Named `PROJ_ACTIVITY_KG_PER_MIN_DAY` (kg/day per activity *minute*) before activity entries could carry their own kcal figure — renamed rather than reinterpreted in place, so an old per-minute calibration under the old key name is simply treated as "not calibrated" instead of silently misapplied to a now-kcal input |
...
| `PROJ_PROTEIN_KG_PER_G_DAY` | `0` (no effect) | Calibrated kg/day per gram/day of protein above/below the protein target band's midpoint (the one figure a regression can center on). Unlike the 4 keys above, this one isn't required for the calibrated formula to apply — an existing calibration from before protein tracking existed keeps working as-is, and only starts factoring in protein after the next Calibrate run |
...
| `FOOD_INSIGHT_LAST_RESULT` | *(none — panel shows a placeholder)* | Written by the Food Insight panel's own **🚀 Send to AI**: the last Groq-generated nutrient-gap report, restored on every load so the panel isn't blank until you regenerate it |
| `FOOD_INSIGHT_LAST_GENERATED_AT` | *(none)* | Timestamp the Food Insight report above was generated — shown as "Last generated ..." under the panel |
| `WELLNESS_INSIGHT_LAST_RESULT` | *(none — panel shows a placeholder)* | Same as `FOOD_INSIGHT_LAST_RESULT`, for the Wellness Insight panel's own **🚀 Send to AI** |
| `WELLNESS_INSIGHT_LAST_GENERATED_AT` | *(none)* | Timestamp the Wellness Insight report above was generated |
| `ACTIVITY_INSIGHT_LAST_RESULT` | *(none — panel shows a placeholder)* | Same as `FOOD_INSIGHT_LAST_RESULT`, for the Activity Insight panel's own **🚀 Send to AI** |
| `ACTIVITY_INSIGHT_LAST_GENERATED_AT` | *(none)* | Timestamp the Activity Insight report above was generated |

Every write to this tab uses `valueInputOption=RAW` and clears the Value cell's number format, rather than the `USER_ENTERED` the entity forms use. This tab is a key/value store where each value is meant to round-trip byte-for-byte, and `USER_ENTERED` reinterprets it the way typing into the cell would — which caused a silently destructive bug worth recording: a `PROJ_CALIBRATED_AT` of `2026-07-30` was parsed into a real date, turning its cell into a DATE-formatted one, and rows appended beside it inherited that format. A coefficient like `0.00031534` is a valid date serial, so it then displayed as `1899-12-30 0:00` and — because the tab is read with `dateTimeRenderOption: 'FORMATTED_STRING'` — read back as that *string* instead of a number. `getSetting()` saw `NaN` and reported the setting as absent, so a correctly-written calibration vanished from the app while sitting intact in the sheet, as would any numeric setting whose cell caught the same format (a `HEIGHT_CM` of 178 reading back as a date is indistinguishable from "height not set"). Because a cell's number format is a separate property from its value, writing RAW alone wouldn't repair an already-affected cell — so each written row's Value format is reset to Automatic in the same operation, which means re-saving a broken setting fixes it in place. A read failure on this tab is also no longer treated as "there are no settings": `loadSettings` keeps the last known-good copy and logs the cause instead of returning `{}` and caching that for the full TTL, which used to wipe every setting in memory on one transient error.
...
│       ├── food-insight.js   # Weight Trend & Forecast's Food Insight button: AI-generated nutrient-gap report
│       ├── activity-insight.js # Activity Insight panel: AI-generated workout-performance report (consistency, volume, per-muscle-group breakdown)
