...
| 23 | `calibration.js` | Weight Trend & Forecast's Calibrate button: fits `charts.js`'s projection formula (energy density, activity, and sleep gains, plus a baseline drift term) to the user's own historical Weight/Calories/Activity/Sleep entries via weighted ridge regression on standardized predictors, with the penalty selected by leave-one-out cross-validation and the intercept left unpenalized, and writes the resulting gains to the `Settings` tab (using the same row-diff/update/append pattern `settings-panel.js` uses) so `charts.js`'s `calcProjection` picks them up on the next render. The save is verified rather than assumed: every written coefficient is read back off the sheet and a partial write is reported by key, since the formula needs all four core gains present or every consumer silently falls back to the generic one. Noise is handled by design rather than by refusing to fit: a training sample is a window of at least `PROJ_CALIBRATION_MIN_INTERVAL_DAYS` (21) whose rate is the least-squares slope through every weigh-in inside it, since differencing whichever two weigh-ins happened to be consecutive is dominated by water/scale swings and squashes the calorie coefficient toward zero — which surfaces as an absurd `1/βcal` energy density. Smoothing the weight series first and differencing *that* is specifically avoided: a centered average reaches outside the window the habit averages cover, which measurably makes the density worse. What the regression produces is what gets saved: `fitCalibration` never substitutes a generic constant for a fitted coefficient, because the whole point of the flow is to yield the user's own numbers and the 📐/📊 toggle already exists for anyone who wants the built-in ones. Implausible results are therefore reported loudly by `validateCalibration` — naming the consequence, e.g. that an extreme density also scales the calculated calorie target through `getCalorieTargetKcal`'s `WEEKLY_FAT_LOSS_KG × density ÷ 7` — rather than overwritten. The only fallback is degeneracy rather than noise: if the penalized system won't solve at all, it fits no sensitivities and leaves the intercept carrying the user's measured baseline drift. So the only remaining hard stop is having too little history to build the minimum number of windows. Also owns the Health Metrics **📐 Calibrated / 📊 Generic formula** toggle, which flips a single flag `charts.js`'s `getCalibratedGains()` reads — no parallel "generic mode" code path exists | `initCalibrationPanel`, `runCalibration`, `fitCalibration`, `saveCalibratedGains`, `resetCalibration`, `initFormulaToggle`, `refreshFormulaToggle` |
...
9. Clicking ⚙️ Calibrate → Run Calibration fits the weighted-least-squares model (energy density, activity burn, sleep sensitivity, protein sensitivity, baseline drift) entirely client-side against the already-loaded `allWellnessEntries` (no API call) and previews the fitted gains plus a confidence score. Each sample is a window of at least 21 days whose rate is the least-squares slope through every weigh-in inside it, so scale noise averages out instead of blocking the fit. Whatever the regression produces is saved as-is — no coefficient is substituted with a generic constant — and unusual values are reported in the modal rather than corrected, since the 📐/📊 formula toggle is the intended way to fall back to the built-in numbers.
...
11. Opening the 💡 Insight modal computes the data preview client-side (selected-range + previous-period aggregation from `allWellnessEntries`, `charts.js`'s `calcProjection`/`getCalibratedGains` for the trajectory line, and BMI if `HEIGHT_CM` is set) — no API call yet, and changing the From/To date range just recomputes this preview.
...
13. Opening the 🥗 Food Insight modal computes an ingredient summary client-side — every Calculate breakdown item across `allWellnessEntries` in the selected custom From/To date range, grouped by name, with a unit-count total converted to grams via `nutrition.js`'s Nutrition Facts lookup — no API call yet.
...
| `SEX` | *(none — falls back to `CALORIE_TARGET_KCAL`)* | `male` or `female` — the Mifflin-St Jeor BMR constant (`+5`/`−161`) used both when calculating a personalized calorie target (see `ACTIVITY_MULTIPLIER` below) and for the Calorie Deficit & Fat Loss chart's per-day maintenance figure, plus the sex term in Body Composition Change's body-fat estimate |
...
| `HEIGHT_CM` | *(none — BMI line/axis and Insight's BMI figure are both omitted)* | Height in cm — enables the Weight Trend & Forecast chart's BMI line/axis and is included (alongside age, current weight, and BMI) in the 💡 Insight report for context. Also one of the three profile keys (with `BIRTH_DATE` and `SEX`) the Calorie Deficit & Fat Loss and Body Composition Change charts both need before they can render |
| `BIRTH_DATE` | *(none — feature skipped)* | ISO date, e.g. `1991-02-12`. Supplies the age term in the Mifflin-St Jeor BMR (the calculated calorie target and the Calorie Deficit & Fat Loss chart's maintenance figure) and in Deurenberg's body-fat estimate (Body Composition Change). Separately, if set and the first `Travel` row is a Departure, credits the home country with the time from this date to that first Departure in the Time Spent by Country breakdown |
...
| `PROJ_CAL_KG_PER_KCAL_DAY` | *(none — generic formula used)* | Calibrated kg/day of weight-trend change per kcal/day of intake above `CALORIE_TARGET_KCAL` (replaces the generic formula's fixed 7,700 kcal/kg). Always the fitted value, never substituted — if it comes out non-positive, `charts.js`'s `kcalPerKgFat()` has no valid density to invert and falls back to 7,700 for the density-consuming charts, while the forecast slope still uses the fitted term |
...
| `PROJ_CALIBRATION_SAMPLES` | *(none)* | Number of multi-week weigh-in windows the calibration was fit against — informational, shown in the Calibrate modal |
| `PROJ_CALIBRATION_ALPHA` | *(written by Calibrate)* | The ridge penalty that fit used, as a ratio of penalty to information — `0` means the fit held up unpenalized, a larger value means it had to be shrunk to predict a held-out weigh-in. Recorded because the coefficients alone can't tell you afterwards how much shrinkage they carry |
