...
- **Food** — ingredients **grouped by Classification**, each group carrying its own ingredient count and calorie/protein totals.
  ...
  - Three more columns after Protein — **Carbohydrate, Fat, TEF** — read off that ingredient's own pulled 🧬 Micronutrients (`estimateMacrosForFoodRow`, `micronutrient-insight.js`) scaled to its range total, the same per-ingredient math the Physique breakdown table uses, just aggregated over the picked range instead of one day. Blank (`—`) on an ingredient with no Micronutrients pulled, never a claimed zero. TEF alone is sent to the model too (`~X kcal TEF` per ingredient line, `estimateTefForFoodRow`) — Carbohydrate/Fat are shown for reference but not sent, since the prompt is a plain-text list, not this table.
...
| F — Breakdown | Text (JSON) | Calculate's per-item breakdown — `{name, amount, calories, protein, carbohydrate, fat, tef, source, noteLine, newRow}` per row, with `carbohydrate`/`fat`/`tef` present only once that ingredient's 🧬 Micronutrients has been pulled. Rendered as one table under the form; the table lists it as an item count with the names on hover |
...
- **Calculate** runs both estimators at once: Consumption fills Breakdown, Calories In and Protein In (`calorie-estimator.js`), and also writes each row's own Carbohydrate/Fat/TEF (`estimateTefBreakdown`, `micronutrient-insight.js`) for whatever already has 🧬 Micronutrients pulled; Workout fills Activity Duration and Calories Out (`activity-estimator.js`). The breakdown table is `renderCalcBreakdown` (`calorie-estimator.js`), drawn into this form via its `target` argument — 💾 banks a new ingredient to `Nutrition` from here too. Editing Consumption never touches the breakdown; it goes stale until you press Calculate again.
- **One breakdown table, one row per ingredient**: Name, Amount, Cal, Pro, Carb, Fat, TEF, Source. Carb/Fat/TEF read "—" rather than a confident 0 on a row whose ingredient has no 🧬 Micronutrients pulled yet. The table is display-shortened, never data-shortened: a `Nutrition` source renders as ✅ (the common, trusted case), and every float (Carb, Fat, and the merged Total row) rounds to 2 decimals for compactness — the saved JSON keeps the fuller value, so an already-saved breakdown re-renders identically without its stored values changing.
...
- **Column widths come from the content, not an even split.** Every column but Name is bounded by its own header — the abbreviated `Cal`/`Pro`/`Carb`/`Fat`/`TEF` headers are each wider than the figure they hold — so each is pinned to its content with `nowrap` and Name takes `width: 100%` of the remainder, on a compact table with tighter padding and no per-row border than the app's other tables carry.
...
- **TEF** (`estimateTefBreakdown`, `micronutrient-insight.js`) reads each Breakdown ingredient's own protein/carb/fat grams straight off the same pulled 🧬 Micronutrients panel `Nutrition`'s Micronutrients column carries, scaled to how much of it was actually logged (the same per-ingredient scaling `aggregateMicronutrientIntake` uses for the Health Insight Micronutrients mode). It writes Carb/Fat/TEF directly onto the matching breakdown row rather than a separate table — a day whose ingredients have none pulled yet is left exactly as it was, reading as "not measured" rather than a measured zero. There's no standalone TEF action any more (single-day or bulk); Calculate runs it as part of the same pass that fills Cal/Pro, so catching up an old day just means re-running Calculate on it.
