...
| `PROJ_CAL_KG_PER_KCAL_DAY` | *(none — generic formula used)* | Calibrated kg/day of weight-trend change per kcal/day of intake above `CALORIE_TARGET_KCAL` (replaces the generic formula's fixed 7,700 kcal/kg). May hold exactly `1/7700` when the fit couldn't produce a physiologically plausible density from the user's history — the Calibrate modal says so when it pins this term, and the other gains are still the user's own |
